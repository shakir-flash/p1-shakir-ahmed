...
And also the dataset doesn't contain any latitude or longitude information to use them to create a map plot. But it has country information in entity variable, which can be used to get the latitude and longitude information. We will be using the `maps` package as an external data source to get the relevant information using `map_data()` function and then merge the obtained data with our data using the country variable. This creates new variables latitude and longitudes of the respective countries.

Then we will us the obtained dataset to plot the map plot using `geom_polygon()` from ggplot. Also, in the data visualization we will attempt to make the plot interactive using `plotly` package allowing users to enhance their understanding. There are also other packages like `rnaturalearth` (for getting geographical information), `ggmap` (for working with maps and spatial data) which we may use based the plot's compatibility.
