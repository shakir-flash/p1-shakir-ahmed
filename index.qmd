...
Further, we used the `forest_area` data from the TidyTuesday repository and `total_forest` dataset from Our World in Data to obtain the total forest coverage trends in the world.

### Approach

To address this question, we implemented a time series visualization highlighting the soybean production in the country of Brazil and the total forest coverage in Brazil and how it the production of soybean correlates to the deforestation trend. We began to scout the data for getting relevant information and noteworthy details. The whole approach can be categorized into Data Preparation, Data pre-processing, Visualizing the data and Animating the plots.

**Data Preparation:**

To generate a time series plot we obtained obtained all the details from the `soybean_use` dataset and cleaned the data from the `total_forest` dataset to standardize the data variables.
...
We used `geom_line` and `geom_point` to visualize the total amount of soybean production in the country of Brazil.

For the plotting of total forest coverage in Brazil and to discern the trends, we used `geom_line` and `geom_line` to visualize the deforestation rates in Brazil. From here, we can compare the plots of Brazil soybean production and how production has affected the deforestation / afforestation in Brazil.

**Animating the plots**

The time series visualization is compiled into an animated GIF using the `gganimate` package. The resulting GIF provides a dynamic overview of the soybean production and deforestation trends, emphasizing the transformations occurring over the years.

### Analysis

Cleaning and pre-processing of the datasets `soybean_use` and `total_forest` for plotting the soybean production and forest coverage details for the country of Brazil.
...
#Making a new column to calculate the total soybean production in Brazil
soybean <- soybean_use |>
...
# Filter data for Brazil for years 1990 to 2013
soybean_brazil <- soybean |>
  filter(entity == "Brazil", year>= 1990&year<=2013)

# Filter data for Brazil forest for years 1990 to 2013
forest_brazil <- forest_area |>
  filter(entity == "Brazil",year>=1990&year<=2013)

#Finding total world forest coverage
...
#Finding actual total coverage for Brazil (percentage * total) (in hectares)
forest_brazil <- forest_brazil|>
  mutate(forest_area_brazil = forest_area.x * forest_area.y / 100)
```

**Plotting the soybean production for the years 1990 to 2013**

```{r q2_plot_soybean, message=FALSE}

# Create a line plot for Brazil soybean usage
plot_soybean_brazil <- ggplot(soybean_brazil, aes(x = year, y = total, color = "Brazil")) +
  geom_line(size = 2) +
  geom_point(color = "black") +
  labs(x = "\nYear", y = "Total (in lb)\n", title = "Total Soybean production in Brazil\n", caption = "Jon Harmon | TidyTuesday") +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_y_continuous(labels = scales::label_number(scale = 1e-06, suffix = "M")) + #Convert large numbers
  scale_color_manual(values = c("Brazil" = "cornsilk3")) +
  scale_x_continuous(limits = c(1990, 2013), breaks = seq(1990, 2013, by = 2)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
plot_soybean_brazil

ggsave(plot_soybean_brazil, #Used to save image as png file
...
**Animating the soybean production graph to show the trends per year**
...
**Plotting the forest coverage in Brazil for years 1990- 2013**
...
**Animating the forest coverage graph to show the trends per year**
...
Valdez, C. (2022). Brazil's Momentum as a Global Agricultural Supplier Faces Headwinds. Published online at ers.usda.gov. Retrieved from: https://www.ers.usda.gov/amber-waves/2022/september/brazil-s-momentum-as-a-global-agricultural-supplier-faces-headwinds/

Pagano, M. & Miransari, M. (2016). "The importance of soybean production worldwide." Retrieved from: https://www.sciencedirect.com/science/article/pii/B9780128015360000013

Song et al., (2021). "Massive soybean expansion in South America since 2000 and implications for conservation." Retrieved from: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8350977/#:\~:text=Direct%20soybean%2Ddriven%20deforestation%20reached,forest%20loss%20during%20this%20period.
